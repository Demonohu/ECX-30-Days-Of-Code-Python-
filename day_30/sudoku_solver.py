...
# Let this list represent a partially filled grid of numbers
# (specifically, of integers ranging from 1 to 9, where '0'
# signifies an empty space in the grid.) as parameters, and
# returns as a 9x9 array, the solution(s) to it. If there are 
# no solutions, return False.
# Exceptions:
# *If an empty list or an invalid list(list with numbers outside
# the 1-9 range, or empty lists, or a list of wrong size, etc.)
# is input, it issues a warning.
# (Hint: Backtracking is one effective method for solving this 
# problem-it is however, not the only method.)
...
# Takes a partially filled-in grid and attempt to assign values to all unassigned locations in
